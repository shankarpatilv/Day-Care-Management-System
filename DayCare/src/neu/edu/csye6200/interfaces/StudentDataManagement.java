...
public class StudentDataManagement implements DataManagement<Student> {
	...
	/**
	 * Search for students that ages are between minimum value and maximum value.
	 * Notice, I use >= and <=
	 */
	@Override
	...
	public List<Student> getDataList(String min, String max) {
		...
		List<Student> stus = new ArrayList<>();
		getDataList().forEach(stu -> {
			if (stu.firstName.compareTo(min) >= 0 && stu.firstName.compareTo(max) <= 0)
				stus.add(stu);
		});
		return stus;
	}
